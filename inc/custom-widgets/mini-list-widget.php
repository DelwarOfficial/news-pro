<?php
if ( ! class_exists( 'News_Record_Mini_List_Widget' ) ) {
	/**
	 * Adds News Record Mini List Widget.
	 */
	class News_Record_Mini_List_Widget extends WP_Widget {
		...
		public function __construct() {
			$mini_list_widget = array(
				'classname'   => 'news-record-widget mini-list-widget',
				'description' => __( 'Retrive Posts List Widgets', 'news-record' ),
			);
			parent::__construct(
				'news_record_mini_list_widget',
				__( 'Artify Widget: Mini List Widget', 'news-record' ),
				$mini_list_widget
			);
		}
		...
		public function form( $instance ) {
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'category' ) ); ?>"><?php esc_html_e( 'Select the category to show posts:', 'news-record' ); ?></label>
				<select id="<?php echo esc_attr( $this->get_field_id( 'category' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'category' ) ); ?>" class="widefat" style="width:100%;">
					<?php
					$categories = news_record_get_post_cat_choices();
