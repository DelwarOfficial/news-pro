<?php
if ( ! class_exists( 'News_Record_Social_Widget' ) ) {
	/**
	 * Adds News Record Social Widget.
	 */
	class News_Record_Social_Widget extends WP_Widget {

		/**
		 * Register widget with WordPress.
		 */
		public function __construct() {
			$news_record_social_widget = array(
				'classname'   => 'news-record-widget social-widget style-1',
				'description' => __( 'Retrive Social Widget', 'news-record' ),
			);
			parent::__construct(
				'news_record_social_widget',
				__( 'Artify Widget: Social Widget', 'news-record' ),
				$news_record_social_widget
			);
		}
		...
		public function form( $instance ) {
			...
			for ( $i = 1; $i <= 4; $i++ ) {
				...
				?>
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'social_icon_label-' . $i ) ); ?>"><?php echo sprintf( esc_html__( 'Social Icon Label %d :', 'news-record' ), $i ); ?></label>
